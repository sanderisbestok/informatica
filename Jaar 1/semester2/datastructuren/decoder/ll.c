...
void list_add(struct list *l, char *d) {
    ...
    /* Else set the pointer of the new element to the current first pointer */
    else {
        (*new_node).pointer = (*l).first;
    }

    /* Set data of new node, and set first as the pointer of the new node*/
    (*new_node).data = d;
    (*l).first = new_node;
    (*l).length++;
}
