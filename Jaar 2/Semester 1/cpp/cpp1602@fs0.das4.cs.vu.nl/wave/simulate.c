...
__global__ void waveStepKernel(double* deviceCur, double* deviceOld double* deviceNext) {
        ...
        double *old_array, double *current_array, double *next_array)
{
    ...
    float* deviceCur = NULL;
    checkCudaCall(cudaMalloc((void **) &deviceCur, n * sizeof(float)));
    if (deviceCur == NULL) {
	...
	for (t = 0; t < t_max; t++) {
		waveStepKernel<<<i_max/threadBlockSize, threadBlockSize>>>(deviceCur, deviceOld, deviceNext);
        checkCudaCall(cudaGetLastError());
        /* Swap buffers */
        temp = deviceOld;
        deviceOld = deviceCur;
