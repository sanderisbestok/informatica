    ...
    double s, double ox, double oy, double oz)
{
    ...
    // Set texture coordinate
    // Longitude divided by 360 degrees, lattitude by 90
    t->x = longitude / 360.0;
    t->y = latitude / 90.0;
    ...
    double r, double g, double b)
{
    ...
    for (longitude = 0; longitude < 360; longitude += 10)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            // Set texture coordinate TODO hoe wordt x gedaan
            p.tcoord[i].x = i % 2 == 0 ? (longitude + 10) / 360.0 : longitude / 360.0;
            p.tcoord[i].y = p.pts[i].y / height;
        }
