...
ray_trace(void)
{
    ...
    vec3    left_top_vector, right_top_vector, right_bottom_vector, left_bottom_vector;
    vec3    final_color, left_top_color, right_top_color, right_bottom_color, left_bottom_color;
    char    buf[128];
    ...
    float left = image_plane_width * -0.5;
    float right = image_plane_width * 0.5;
    float top = image_plane_height * -0.5;
    float bottom = image_plane_width * 0.5;

    vec3 direction_vector;

    /* Loop over all the pixels in the framebuffer */
    for (j = 0; j < framebuffer_height; j++)
    {
        for (i = 0; i < framebuffer_width; i++)
        {
            /* Calculate window view transformation (buffer to window view) */
            float us = left + (right - left) * ((i + 0.5) / framebuffer_width);
            float vs = bottom + (top - bottom) * ((j + 0.5) / framebuffer_height);

            float offset_right = 0.25 * (right - left) / framebuffer_width;
            float offset_up = 0.25 * (top - bottom) / framebuffer_height;

            /* Calculate direction vector */
            direction_vector = v3_add(v3_add(v3_multiply(right_vector, us), v3_multiply(up_vector, vs)), forward_vector);

            if (do_antialiasing) {
                left_top_vector = v3_add(direction_vector,
                                         v3_add(v3_multiply(right_vector, -offset_right),
                                                v3_multiply(up_vector, offset_up)));
                right_top_vector = v3_add(direction_vector,
                                          v3_add(v3_multiply(right_vector, offset_right),
                                                 v3_multiply(up_vector, offset_up)));
                right_bottom_vector = v3_add(direction_vector,
                                             v3_add(v3_multiply(right_vector, offset_right),
                                                    v3_multiply(up_vector, -offset_up)));
                left_bottom_vector = v3_add(direction_vector,
                                            v3_add(v3_multiply(right_vector, -offset_right),
                                                   v3_multiply(up_vector, -offset_up)));

                left_top_color = ray_color(0, scene_camera_position, left_top_vector);
                right_top_color = ray_color(0, scene_camera_position, right_top_vector);
                right_bottom_color = ray_color(0, scene_camera_position, right_bottom_vector);
                left_bottom_color = ray_color(0, scene_camera_position, left_bottom_vector);

                final_color = v3_multiply(v3_add(v3_add(left_top_color, right_top_color),
                                                 v3_add(right_bottom_color, left_bottom_color)),
                                          0.25);
            } else {
                final_color = ray_color(0, scene_camera_position, direction_vector);
            }

            /* Output pixel color */
            put_pixel(i, j, final_color.x, final_color.y, final_color.z);
        }
